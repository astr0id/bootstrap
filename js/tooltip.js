...
var Tooltip = function (element, opt_config) {
  ...
  if (this.config.viewport) {
    ...
    /** @private {Element} */
    this._viewport = document.querySelector(this.config.viewport.selector || this.config.viewport)
...
Tooltip.prototype['toggle'] = function (opt_event) {
  ...
  if (opt_event) {
    context = $(opt_event.currentTarget).data(this.constructor._DATA_KEY)

    if (!context) {
      context = new this.constructor(opt_event.currentTarget, this._getDelegateConfig())
      $(opt_event.currentTarget).data(this.constructor._DATA_KEY, context)
    }
  }

  context.getTipElement().classList.contains(Tooltip._ClassName.IN) ?
...
Tooltip.prototype['destroy'] = function () {
  ...
  this['hide'](function () {
    $(this.element)
      ...
      .removeData(this.constructor._DATA_KEY)
  }.bind(this))
}
...
Tooltip.prototype['show'] = function () {
  var showEvent = $.Event(this.constructor._Event.SHOW)
  ...
  if (this.isWithContent() && this._isEnabled) {
    ...
    var tipId = Bootstrap.getUID(this.constructor._NAME)
    ...
    if (this.config.animation) {
      tip.classList.add(Tooltip._ClassName.FADE)
    }
    ...
    $(tip).data(this.constructor._DATA_KEY, this)

    if (this.config.container) {
      document.querySelector(this.config.container).appendChild(tip)
    } else {
    ...
    var complete = function () {
      ...
      $(this.element).trigger(this.constructor._Event.SHOWN)
      this.hoverState = null

      if (prevHoverState == 'out') this._leave(null, this)
    }.bind(this)

    Bootstrap.transition && this._tip.classList.contains(Tooltip._ClassName.FADE) ?
      $(this._tip)
...
Tooltip.prototype['hide'] = function (callback) {
  var tip       = this.getTipElement()
  var hideEvent = $.Event(this.constructor._Event.HIDE)
  ...
  var complete  = function () {
    ...
    $(this.element).trigger(this.constructor._Event.HIDDEN)
  ...
  tip.classList.remove(Tooltip._ClassName.IN)

  if (Bootstrap.transition && this._tip.classList.has(Tooltip._ClassName.FADE)) {
    $(tip)
      .one(Bootstrap.TRANSITION_END, complete)
      .emulateTransitionEnd(Tooltip._TRANSITION_DURATION)
  } else {
...
Tooltip.prototype.getArrowElement = function () {
  return (this.arrow = this.arrow || this.getTipElement().querySelector(Tooltip._Selector.TOOLTIP_ARROW))
}
...
Tooltip.prototype.setContent = function () {
  ...
  tip.querySelector(Tooltip._Selector.TOOLTIP_INNER)[this.config.html ? 'innerHTML' : 'innerText'] = title

  tip.classList.remove(Tooltip._ClassName.FADE)
  tip.classList.remove(Tooltip._ClassName.IN)
...
Tooltip.prototype._getCalculatedAutoPlacement = function (isWithAutoPlacement, placement, position, actualWidth, actualHeight) {
  if (isWithAutoPlacement) {
    ...
    var container         = this.config.container ? document.querySelector(this.config.container) : this.element.parentNode
...
Tooltip.prototype._applyCalculatedPlacement = function (offset, placement) {
  ...
  tip.classList.add(Tooltip._ClassName.IN)
...
Tooltip.prototype._enter = function (opt_event, opt_context) {
  var context = opt_context || $(opt_event.currentTarget).data(this.constructor._DATA_KEY)
