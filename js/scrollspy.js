...
ScrollSpy.prototype._activate = function (target) {
  ...
  for (var i = parentListItems.length; i--;) {
    parentListItems[i].classList.add(ScrollSpy._ClassName.ACTIVE)

    var itemParent = parentListItems[i].parentNode

    if (itemParent && itemParent.classList.contains(ScrollSpy._ClassName.DROPDOWN_MENU)) {
      var closestDropdown = $(itemParent).closest(ScrollSpy._Selector.LI_DROPDOWN)[0]
      closestDropdown.classList.add(ScrollSpy._ClassName.ACTIVE)
    }
  }
...
ScrollSpy.prototype._clear = function () {
  ...
  for (var i = activeParents.length; i--;) {
    activeParents[i].classList.remove(ScrollSpy._ClassName.ACTIVE)
  }
}
...
$(window).on('load.bs.scrollspy.data-api', function () {
  var scrollSpys = document.querySelectorAll(ScrollSpy._Selector.DATA_SPY)
